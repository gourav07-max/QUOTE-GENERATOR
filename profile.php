...
if (isset($_POST['submit'])){
   ...
   $sql = "INSERT INTO history (sender, receiver, amount) VALUES (?,?,?)";
   $stmt= $pdo->prepare($sql);
   $stmt->execute([$sender, $receiver, $amount]);

   $sql = "UPDATE users SET amount=amount-$amount WHERE users='$sender'";
   $stmt= $pdo->prepare($sql);
   $stmt->execute();

   $sql = "UPDATE users SET amount=amount+$amount WHERE users='$receiver'";
   $stmt= $pdo->prepare($sql);
   $stmt->execute();
 ?>
 <script>
                           ...
                           <hr style="border:1px solid white;">
                                <div class="panel panel-default" style="color:  black;" >
                                   <div class="panel-body">
                                      <p><b>Name:</b> <?php echo htmlentities($row['users']); ?></p>
            <p><b>Email Id:</b> <?php echo htmlentities($row['email']); ?></p>
            <p><b>Balance:</b> <?php echo htmlentities($row['amount']); ?></p>   
            <p><button class="btn btn-dark btn-xs view_data" data-toggle="modal" data-target="#transfer<?php echo $row['id'] ?>">Transfer Money</button></p> 
                                   </div>
                                </div>
            </div>
            </div>
                            ...
                            <select name="receiver" class="form-control">
                                <?php 
                                   ...
                                   foreach($rows as $row ) { ?>
                                     ...
                                     <option><?php echo $row['users'] ?></option>
